<template>
  <div>
    <el-form :model="pageForm" :rules="pageFormRules" ref="pageForm" label-width="80px">
      ...
      <el-form-item label="数据模型" prop="dataUrl">
...
<script>
    ...
    export default {
        ...
        mounted(){
           ...
           cms_api.cms_get(this.pageForm.pageId).then((res)=>{
               if(res.success){
                ...
                this.$refs.pageForm.validate((valid) => {
                    if (valid) {
                        this.$confirm("确认提交吗?", "提示", {}).then(() => {
                            cms_api.cms_edit(this.pageForm.pageId,this.pageForm).then((res) => {
                                if (res.success) {
                                    this.$message({message: "提交成功", type: "success"})
                                } else {
                                    this.$message({message: "提交失败", type: "failed"})
                                }
                            })
                        })
                    } else {
